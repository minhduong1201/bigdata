...
list_code = pd.read_excel("code_stock.xlsx")
...
def get_data(code="VIC", page=1):
    ...
    URL = "https://s.cafef.vn/Lich-su-giao-dich-{}-1.chn".format(code)
    hdr = {
...
def get_all_data(code="VIC"):
    available = True
    try:
        df = pd.read_csv('data-all/{}.csv'.format(code))
    ...
    df.to_csv('{}.csv'.format(code), index=False)
